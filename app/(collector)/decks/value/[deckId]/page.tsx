...
interface DeckCardValue { name: string; qty: number; unitPrice: number | null; lineValue: number | null }
interface DeckValue {
  deckId: string; deckName: string; commander: string | null;
  totalValue: number; cardCount: number; cardsMatched: number; cards: DeckCardValue[];
}
...
export default function DeckValueDetailPage() {
  const params = useParams<{ deckId: string }>();
  const deckId = params.deckId;
  const [value, setValue] = useState<DeckValue | null | undefined>(undefined);
  const [computedAt, setComputedAt] = useState<string | null>(null);
  const [sort, setSort] = useState<Sort>('value_desc');
  const [filter, setFilter] = useState<'all' | 'missing'>('all');

  const load = useCallback(async () => {
    const res = await fetch(`/api/decks/${deckId}/value`).then(r => r.json());
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-8 text-zinc-100">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-2">
        <div>
          <h1 className="text-2xl font-bold">{value?.deckName ?? 'Deck'}</h1>
          {value?.commander && <p className="text-sm text-zinc-500 mt-0.5">Commander: {value.commander}</p>}
        </div>
        <BackButton fallbackHref="/decks/value" />
      </div>
      ...
      {value && (
        <>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 my-4">
            <p className="text-[10px] uppercase tracking-wide text-zinc-500 mb-1">Estimated total</p>
            <p className="text-2xl font-bold text-amber-400">{fmtUsd(value.totalValue)}</p>
            <p className="text-xs text-zinc-500 mt-1">
              {value.cardsMatched} of {value.cardCount} cards priced
              {value.cardsMatched < value.cardCount && ' — the rest have no tracked printing under that exact name'}.
            </p>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            ...
            <div className="max-h-[420px] overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {visible.map(c => (
                    <tr key={c.name} className="border-b border-zinc-800/60 last:border-0 hover:bg-zinc-800/40">
