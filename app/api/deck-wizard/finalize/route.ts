...
export async function POST(req: NextRequest) {
  ...
  const { sessionId, deckName, cards, format: bodyFormat, archetype: bodyArchetype, commander: bodyCommander, rubricScore } = await req.json() as {
    sessionId?: string;
    deckName: string;
  ...
  await run(
    `INSERT INTO decks (id, "userId", name, format, strategy, cards, commander, rubric_score, rubric_scored_at, "deckType", "createdAt", "updatedAt")
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'paper', NOW(), NOW())`,
    [
      deckId, userId, deckName, format, archetype ?? null, JSON.stringify(cards),
      bodyCommander ?? null,
      rubricScore ? JSON.stringify(rubricScore) : null,
      rubricScore ? new Date().toISOString() : null,
    ]
  );
